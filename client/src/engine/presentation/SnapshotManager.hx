...
import engine.infrastructure.state.GameModelState.GameStateMemento;
...
class SnapshotManager {
    ...
    public function store(tick: Int, memento: GameStateMemento): Void {
        final snapshot = {tick: tick, memento: cloneMemento(memento)};
    ...
    public function load(tick: Int): GameStateMemento {
        for (snapshot in snapshots) {
            if (snapshot.tick == tick) {
                return cloneMemento(snapshot.memento);
            }
        }
        return null;
    }
    ...
    private function cloneMemento(memento: GameStateMemento): GameStateMemento {
        final serialized = Serializer.run(memento);
        return cast Unserializer.run(serialized);
    }
}
